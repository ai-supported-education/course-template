...
Новое слово в заголовке не считается введённым самим заголовком: дайте ему
короткое рабочее определение в первых фразах. До вывода из примера назовите роли
объекта и полей (`task`, `id`, `title`), а расплывчатую операцию вроде «обычной
копии» замените конкретной формой (`{ ...task }`, `Object.keys`, команда). Нельзя
считать causal example полным, если текст сообщает желаемую политику, но не
показывает initial state, действие и наблюдаемый результат исходного поведения.
...
Проверьте маршрут буквально сверху вниз. Команда не может выполняться до
прогноза, если evidence требует прогноз «до первого запуска»; baseline не может
появляться после исправления, а review — до зелёного check. Для каждого
редактируемого artifact (`answers.json`, журнал наблюдений, отчёт) learner-facing
текст обязан показать путь и структуру, достаточные для заполнения, не раскрывая
правильный ответ. Если DONE требует agent review, отдельно объясните, что
`session:check` не запускает агента, как получить `session:review` packet и кто
записывает фактический verdict.
