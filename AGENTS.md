...
Если rubric требует фактические red/green запуски, терминальный вывод сам по себе
не является сохраняемым evidence. Дайте учащемуся `evidence.md` либо явные разделы
в worksheet/notes для версии среды, команды, exit code и релевантного наблюдения
до и после исправления. Заполнять expected output вместо собственного запуска
нельзя.
