...
Если rubric проверяет, что красный и зелёный результаты были фактически получены,
создайте learner-editable evidence artifact: версия среды, точная команда, exit
code и короткий релевантный фрагмент каждого запуска. Требование «сохранить
baseline» без файла или именованного раздела оставляет reviewer только эфемерный
терминал и блокирует воспроизводимую проверку.
...
runtime metadata, timestamp и hashes, но не patch, вывод команды или готовый ответ. Proof входит в
